...
impl GpuMemoryManager {
    ...
    pub fn allocate(&self, size: u64) {
        let current = self.current_usage.fetch_add(size, std::sync::atomic::Ordering::Relaxed) + size;
...
impl GpuBuffer {
    ...
    pub async fn read_data<T: bytemuck::Pod + Clone>(&self, device: &Device, queue: &Queue) -> Result<Vec<T>> {
